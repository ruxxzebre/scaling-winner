...
if not TheFixesPreventer.heist_mallcrasher_continuous_gas_sound then
	for i = 301571, 301576, 1 do
		if i ~= 301573 then -- Does not exist
			local element = managers.mission:get_element_by_id(i)
			if element then
				element._values.use_instigator = false -- STOP THE CONTINUOUS EARRAPE, thx
			end
		end
	end
end

if Network:is_client() or TheFixesPreventer.heist_mallcrasher_secure_zone then
	return
end
